...
#   - Uninstall:         curl -sSL <URL_TO_SCRIPT> | sudo bash -s -- --uninstall
#
...
set_profile_datacenter_10g() {
    ...
    # Ring buffers are set to max in the tune_nic.sh script for this profile
    export ETHTOOL_SETTINGS=(
        "adaptive-rx on"
        "adaptive-tx on"
    )
...
EOF
    ...
    # Dynamic profile-based configuration
...
apply_nic_tuning() {
    ...
    # Store the chosen profile for the boot script
...
EOF
    ...
    # Run the tuning script immediately
...
ask_questions() {
    ...
    select choice in "auto (Balanced BBR)" "lan_low_latency (Low Delay)" "wan_throughput (Bulk Transfer)" "datacenter_10g (10G+ Networks)"; do
        case $choice in
            ...
            "lan_low_latency (Low Delay)") set_profile_lan_low_latency; break ;;
            "wan_throughput (Bulk Transfer)") set_profile_wan_throughput; break ;;
